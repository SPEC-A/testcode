...
contract certificate {
    ...
    function verification(bytes32 hash) public view returns(bool){
        if(hash == certificates[msg.sender].certhash){
            return true;
        }
        else{
            return false;
    ...
    function verification2(string memory name, string memory birth, uint nbefore, uint nafter, bytes32 id) public view returns(bool){
